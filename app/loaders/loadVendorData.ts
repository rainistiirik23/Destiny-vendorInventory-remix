import {
  getCurrentVendorSales,
  getDiscordAccountId,
  getAllVendorSales,
  getVendorSalesWithUsersWishListedSales,
} from "~/utils/requests";
...
export async function loadCurrentVendorSalesWithDiscordId(code: string) {
  const discordAccountId = await getDiscordAccountId(code);
  return { data: { discordAccountId } };
}
